<?php
// contact-submit.php — handles the general Contact Us form submissions via SMTP
...
if ($name === '' || $phone === '' || $email === '' || $province === '' || $inquiryType === '') {
...
$typeLabel = $typeLabels[$inquiryType] ?? ucfirst($inquiryType);
...
$fields = [
    ...
    ['icon' => '&#128172;', 'label' => 'Inquiry Type',  'value' => $typeLabel],
];
if ($otherDetail !== '') {
    $fields[] = ['icon' => '&#128221;', 'label' => 'Message', 'value' => $otherDetail];
}
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 12px 40px rgba(10,35,66,0.12);">
                    <tr>
                        <td style="background:linear-gradient(135deg,#0A2342 0%,#16335c 100%);padding:36px 32px 30px;text-align:center;">
                            ...
                            <h1 style="color:#ffffff;font-size:21px;margin:0 0 6px;font-weight:700;">New Contact Inquiry</h1>
                            <p style="color:#c49553;font-size:12.5px;letter-spacing:1px;text-transform:uppercase;margin:0;font-weight:600;">Rajdoot Paints Nepal &middot; {$safe($typeLabel)}</p>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style="padding:4px 26px 28px;">
                            <p style="color:#8a8a8a;font-size:12px;margin:0;text-align:center;">Submitted via the Contact form on the Rajdoot Paints website.</p>
                        </td>
                    </tr>
                    <tr>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 12px 40px rgba(10,35,66,0.12);">
                    <tr>
                        <td style="background:linear-gradient(135deg,#0A2342 0%,#16335c 100%);padding:40px 32px 32px;text-align:center;">
                            ...
                            <p style="color:#c49553;font-size:12.5px;letter-spacing:1px;text-transform:uppercase;margin:0;font-weight:600;">Inquiry Received</p>
                        </td>
                    </tr>
                    <tr>
                        <td class="gold-divider" style="height:3px;"></td>
                    ...
                    <tr>
                        <td style="padding:28px 26px 6px;">
                            <p style="color:#333;font-size:15px;line-height:1.65;margin:0 0 22px;">
                                We've received your <strong>{$safe($typeLabel)}</strong> inquiry. Our team will carefully review your message and get back to you as soon as possible.
                            </p>
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                {$fieldBoxesHtml}
                            </table>
...
try {
    ...
    $ownerMail->Subject = "New Contact Inquiry — {$typeLabel}";
    ...
    $replyMail->Subject = 'Thank you for contacting Rajdoot Paints Nepal';
    $replyMail->Body    = $userHtml;
    $replyMail->send();

    respond(true, 'Inquiry submitted successfully.');
} catch (Exception $e) {
    http_response_code(500);
    respond(false, 'Something went wrong while sending your inquiry. Please try again later.');
}
